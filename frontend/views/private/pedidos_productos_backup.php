                    ...
                    <div class="row mb-3">
                        <div class="col-12">
                            ...
                            <div class="btn-group w-100" role="group">
                                <input type="radio" class="btn-check" name="tipo_pedido" id="tipo_habitacion" value="habitacion" checked>
                                <label class="btn btn-outline-primary" for="tipo_habitacion">
                                    <i class="fas fa-bed me-2"></i>Pedido de Habitación
                                </label>
                                
                                <input type="radio" class="btn-check" name="tipo_pedido" id="tipo_directa" value="directa">
                                <label class="btn btn-outline-primary" for="tipo_directa">
                                    <i class="fas fa-shopping-cart me-2"></i>Venta Directa
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="seccionDirecta" style="display: none;">
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="referencia_venta" readonly>
...
function cargarHabitaciones() {
    ...
    $.get('api/endpoints/reservas.php?estado=confirmada', function(reservasData) {
        ...
        $.get('api/endpoints/habitaciones.php', function(habitacionesData) {
            ...
            habitacionesActivas.forEach(function(habitacion) {
                ...
                if (reserva.observaciones) {
                    ...
                    try {
                        ...
                        if (acompanantesMatch && acompanantesMatch[1]) {
                            ...
                            acompanantes.forEach(function(acompanante, index) {
...
function agregarProducto() {
    const productoHtml = '<div class="row mb-2 producto-item">' +
        '<div class="col-md-3">' +
            '<select class="form-select producto-select" required onchange="actualizarPrecio(this)">' +
                '<option value="">Seleccione producto...</option>' +
            '</select>' +
...
function guardarPedido(e) {
    ...
    console.log('Guardando pedido:', { tipoPedido, habitacionId, referenciaVenta, notas });
    ...
    $('.producto-item').each(function() {
        ...
        // Para ventas directas, no se requiere cliente
        if (tipoPedido === 'directa') {
            if (!productoId || cantidad <= 0 || precio <= 0) {
                valido = false;
                console.log('Producto inválido (venta directa):', { productoId, cantidad, precio });
                return false;
            }
            ...
            detalles.push({
                ...
                cliente_id: null // Venta directa no tiene cliente
            });
        } else {
            // Pedido de habitación: requiere cliente
            if (!productoId || cantidad <= 0 || precio <= 0 || !clienteProductoId) {
                valido = false;
                console.log('Producto inválido (pedido habitación):', { productoId, cantidad, precio, clienteProductoId });
                return false;
            }
...
$('input[name="tipo_pedido"]').change(function() {
    ...
    if (tipoPedido === 'directa') {
        // Ocultar sección de habitación y mostrar referencia
        ...
        // Ocultar selects de cliente en productos existentes
        $('.cliente-select').each(function() {
            $(this).parent().hide();
        });
    } else {
        // Mostrar sección de habitación y ocultar referencia
        $('#seccionHabitacion').show();
        $('#seccionDirecta').hide();
        
        // Mostrar selects de cliente en productos existentes
        $('.cliente-select').each(function() {
            $(this).parent().show();
        });
    }
