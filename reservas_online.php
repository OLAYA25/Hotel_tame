...
// Incluir el controlador de reservas web
...
// Procesar búsqueda de habitaciones
$searchResults = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['checkin']) && isset($_GET['checkout'])) {
    try {
        ...
        $searchResults = $webBookingService->searchAvailableRooms($searchParams);
...
// Obtener tipos de habitación disponibles desde la base de datos
$roomTypes = ['estandar', 'suite', 'deluxe', 'familiar'];

// Obtener algunas habitaciones para mostrar como ejemplo
$sampleRooms = [];
try {
    $db = Database::getConnection();
    $sql = "SELECT id, numero, tipo, capacidad, precio_base as precio_noche, descripcion 
            FROM habitaciones 
            WHERE estado = 'disponible' AND deleted_at IS NULL 
            ORDER BY precio_base ASC LIMIT 6";
    $stmt = $db->query($sql);
    $sampleRooms = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    // Si hay error, mostrar habitaciones de ejemplo
    $sampleRooms = [
